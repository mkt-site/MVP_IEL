...
const perguntas = {
  nome: "Olá 👋 Seja bem-vindo ao IEL Santa Catarina. Qual é o seu nome?",
  empresa: "Qual o nome da empresa ou instituição?",
  whatsapp: "Qual seu WhatsApp para contato?",
  categoria: "Qual serviço você procura?",
  necessidade: "Conte um pouco mais sobre sua necessidade.",
  urgencia: "Qual a urgência da demanda?",
  porte: "Qual o porte da empresa?"
};
...
function enviar() {
  ...
  if (etapa < campos.length) {
    ...
    if (campos[etapa] === "porte") mostrarBotoes(portes);
  } else {
    finalizar();
  }
}
...
function calcularScore() {
  ...
  const porte = (lead.porte || "").toLowerCase();

  if (urgencia.includes("agora")) score += 50;
  else if (urgencia.includes("seman")) score += 30;
  else if (urgencia.includes("mes")) score += 15;

  if (porte.includes("grande")) score += 40;
  else if (porte.includes("média")) score += 25;
  else if (porte.includes("pequena")) score += 15;
  else if (porte.includes("micro")) score += 10;
...
Em breve entraremos em contato através dos canais informados.
      ...
      bot("❌ Erro ao enviar. Tente novamente mais tarde.");
